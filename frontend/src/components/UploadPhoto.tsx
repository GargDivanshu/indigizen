...
const UploadPhoto = () => {
  ...
  const getCanvasDimensions = () => {
    ...
    console.log({ x: window.innerWidth * 0.7, y: window.innerHeight } + "getcanvas");
  };
  ...
  useEffect(() => {
    const updateDimensions = () => {
      ...
      console.log({ x: window.innerWidth * 0.7, y: window.innerHeight }, "updateDimensions")
    };
  ...
  const generatePDF = async () => {
    try {
      ...
      const konvaCanvas = imageRef.current.toCanvas();
      const konvaDataUrl = konvaCanvas.toDataURL('image/jpeg');
      const konvaBlob = await fetch(konvaDataUrl).then((response) => response.blob());
      ...
      const konvaWidth = konvaImage.width;
      const konvaHeight = konvaImage.height;
      const isLandscape = konvaWidth > konvaHeight;
      const scaleFactor = Math.max(width / (isLandscape ? konvaHeight : konvaWidth), height / (isLandscape ? konvaWidth : konvaHeight));
      const scaledWidth = konvaWidth * scaleFactor;
      const scaledHeight = konvaHeight * scaleFactor;
      ...
      page.drawImage(konvaImage, {
        x: isLandscape ? 0 : x,
        y: isLandscape ? 0 : y,
        width: isLandscape ? height : scaledWidth,
        height: isLandscape ? width : scaledHeight,
      });
  
      // Scale the draggableData positions based on the scaling factors
      const scaledDraggableData = draggableData.map((data) => {
        const { value, position } = data;
        const scaledPosition = {
          x: position.x * scaleFactor + (isLandscape ? 0 : x),
          y: position.y * scaleFactor + (isLandscape ? 0 : y),
        };
        return { value, position: scaledPosition };
      });
  
      // Add the scaled draggable data to the PDF page
      for (const data of scaledDraggableData) {
        const { value, position } = data;
  ...
  //     formData.append('draggableData', JSON.stringify(draggableData));
  
  //     const response = await axios.post('http://localhost:8080/api/v1/toPDF', formData, {
  //       responseType: 'blob',
  //     });
  ...
  //   const canvasDataURL = await captureCanvas(stageRef);
  ...
  //   const html = '...'; // The HTML content you want to include in the PDF
  //   const canvasDataURL = await captureCanvas(stageRef);
  
  //   // Send a request to the API endpoint with the HTML content and canvas data URL
  //   const response = await fetch('http://localhost:8080/api/v1/toPDF', {
  //     method: 'POST',
  //     headers: {
  //       'Content-Type': 'application/json',
  //     },
  //     body: JSON.stringify({ html, canvasDataURL }),
  //   });
  
  //   if (response.ok) {
  //     console.log('Response received'); // Add console log here
  ...
  const handleFileInputChange = async (event) => {
    ...
    image.onload = () => {
      ...
      setNewImageDimensions({ width: newWidth, height: newHeight });
      console.log("New Image Dimensions:", newWidth, newHeight + "this is handleInput ")
      setOffset({ x: offsetX, y: offsetY });
  
      // Send the original image and scaling factor to the backend
      // const scalingFactor = newWidth / image.width;
...
const handleMouseMove = (event, id) => {
  ...
  console.log("draggableData =", draggableData, "this is handleMouseMove");
  setDraggableData((prev) => {
    return prev.map((data) => {
...
const handleSave = async () => {
  try {
    ...
    const containerElements = Array.from(containerRef.current.children) as HTMLElement[];
    ...
    containerElements.forEach((element) => {
      ...
      compositeContext.fillText(value, positionX + offset.x, stageHeight - (positionY + offset.y));
    });
              ...
              >
                ...
                {draggableData.map((data) => (
                  ...
                  >
                    ...
                    >
                      <span className="table-cell align-middle text-center my-auto">{data.value}</span>
                    </div>
